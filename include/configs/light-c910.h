...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"aon_ram_addr=0xffffef8000\0" \
	"audio_ram_addr=0xffc0000000\0" \
	"env_load_addr=0x45000000\0" \
	"env_import=if test -e mmc 1:1 config.txt; then load mmc 1:1 ${env_load_addr} config.txt; else load mmc 0:2 ${env_load_addr} config.txt ; fi; env import -t ${env_load_addr} ${filesize};\0" \
	"fwaddr=0x10000000\0"\
	"kdump_buf=500M\0" \
	"load_aon=if test -e mmc 1:1 light_aon_fpga.bin; then load mmc 1:1 $fwaddr light_aon_fpga.bin; else load mmc 0:2 $fwaddr light_aon_fpga.bin; fi; cp.b $fwaddr $aon_ram_addr $filesize\0" \
